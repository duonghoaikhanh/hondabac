	...
	**/
	...
	function _ordering_form ()
	{
		...
		if (isset($ttH->post['do_submit']) && $ttH->post['do_submit'] == 'save') {
			...
			$arr_pro = $ttH->load_data->data_table ('product', 'item_id', 'item_id,picture,price_buy,price_s,price_l,title,friendly_link ', " is_show=1 and lang='".$ttH->conf['lang_cur']."' and find_in_set(item_id,'".@implode(',',$arr_cart_list_pro)."')>0 order by show_order desc, date_create asc");
				...
				if($ok){
					...
					if(is_array($arr_cart) && count($arr_cart) > 0){
						foreach($arr_cart as $cart_id => $row) {
							...
							$col['price_buy'] = 0;
							if($col['quantity'] >1){
								$col['price_buy'] = $row_pro['price_s'];
							}elseif($col['quantity'] ==1){
								$col['price_buy'] = $row_pro['price_l'];
							}else{
								$col['price_buy'] = $row_pro['price_buy'];
							}
	...
	function do_ordering_complete ()
	{
		...
		'product', 
		'item_id', 
		'item_id,picture,price_buy,price_s,price_l,title,friendly_link', 
		" is_show=1 
			and lang='".$ttH->conf['lang_cur']."' 
