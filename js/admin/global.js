...
function do_submit_comment_status(action) {
    document.manage.do_action.value = action;
    if (selected_item_comment_status()) {
        document.manage.submit();
    }
}

function selected_item_comment_status() {
    var ok = 0;
    $("form#manage .table_row select").each(function () {
        var c = $(this).change();
        if (c) {
            ok = 1;
        }
