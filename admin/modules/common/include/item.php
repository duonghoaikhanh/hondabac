...
class sMain_sub extends sMain {
    ...
    function do_access_comment($list_access = "") {
        global $ttH;
        ...
        foreach ($list_access as $key => $value) {
            $info_ex = explode(",", $value);
            $sql = "update comment set status = " . $info_ex[1] . " where id = " . $info_ex[0] . "";
            $ok = $ttH->db->query($sql);
        }
        if ($ok) {
            $mess = $ttH->html->html_alert($ttH->lang["global"]["edit_success"], "success");
        } else {
            $mess = $ttH->html->html_alert($ttH->lang["global"]["edit_false"], "error");
        }
        return $mess;
    ...
    function do_manage($is_show = "") {
        ...
        if (isset($ttH->input['do_action'])) {
            ...
            switch ($ttH->input["do_action"]) {
                ...
                case "do_access_comment":
                    if (isset($ttH->input['status'])) {

                        $list_access = $ttH->input['status'];

                    }

                    $err = $this->do_access_comment($list_access);
        ...
        if ($num = $ttH->db->num_rows($result)) {
            ...
            while ($row = $ttH->db->fetch_row($result)) {
                ...
                if (isset($row['status'])) {
                    $row['status_text'] = ($row['status'] == 1) ? $ttH->lang['comment']['daduyet'] : $ttH->lang['comment']['chuaduyet'];
                    $row['selected1'] = ($row['status'] == 1) ? "selected" : "";
                    $row['selected0'] = ($row['status'] == 0) ? "selected" : "";
                }
                if (isset($row['info_customer'])) {
                    $info_customer = json_decode($row['info_customer']);
                    $row['author_name'] = $info_customer->full_name;
                    $row['author_email'] = $info_customer->email;
                }
