...

{
...

	{
		...
		if($num > 0) {
...

			{
				...
				$row['link'] = isset($row['link']) ? $row['link'] : $row['link'];
				$row['title'] = isset($row['title']) ? $row['title'] : $row['title'];
	...
	function newsest ($num_show = 10)
	{
		...
		$output = '<div class="newsest"><div class="home-title-line">
										<h4 class="home-title"><span>Tin tức mới nhất</span></h4></div>
                <marquee behavior="scroll" direction="up" onmouseout="this.start();" onmouseover="this.stop();" scrollamount="3" scrolldelay="0" style="height: 222px;">';
		...
		if ($num = $ttH->db->num_rows($result))
		{
...

			{
				...
				$row["picture"] = $ttH->func->get_src_mod($row["picture"], 60, 60, 1, 1);
				...
				$ttH->temp_box->parse("newsest.row");


			}
			$ttH->temp_box->parse("newsest");
		}
		$output .= $ttH->temp_box->text("newsest");
		$output .= '</marquee></div>';
	...
	function list_home_news ($start=0,$num_show = 6)
	{
		...
		$output = $ttH->temp_box->text("list_home_news");
		return array(
			'stop_more' => $stop_more,
			'output'=>$output
		);
...

  {
	...
	function fanpage_facebook (){
		...
		$output = "";
		$arr_fanpage = explode(";", $share_page);
		foreach ($arr_fanpage as $key => $value) {
			$output .= "<div class='fanpage'>";
            ...
            <div class="fb-page" data-href="'.$value.'" data-tabs="timeline" data-width="300" data-height="214" data-small-header="false" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true"><div class="fb-xfbml-parse-ignore"><blockquote cite="'.$value.'"><a href="'.$value.'">Facebook</a></blockquote></div></div>';
            $output .= "</div>";
